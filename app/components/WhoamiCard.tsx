...
export default function WhoamiCard({ language = 'EN' }: WhoamiCardProps) {
  ...
  const motion = reducedMotion ? 'duration-0' : 'transition-all duration-200 ease-out';
    ...
    <>
      ...
      {cardState !== 'closed' && (
        <div
          ...
          className={`fixed z-50 top-3 left-3 sm:top-6 sm:left-6 flex w-28 sm:w-[188px] flex-col overflow-hidden rounded-2xl border border-accent/30 bg-bg-surface shadow-[0_8px_24px_rgba(26,29,46,0.12)] ${motion}`}
        >
          {/* Titlebar */}
          <div className="flex items-center justify-end border-b border-accent/15 px-2 py-1.5">
            <button
              ...
              className={`focus-ring flex h-5 w-5 items-center justify-center rounded-full text-text-muted hover:bg-accent-soft hover:text-accent ${motion}`}
            >
              <svg
                width="10"
                height="10"
          ...
          <button
            ...
            className={`focus-ring w-full border-t border-accent/15 bg-bg-surface px-3 py-2 font-sans text-[10px] font-semibold tracking-widest text-[#1f4d3a] hover:bg-accent-soft sm:text-xs ${motion}`}
          >
            {aboutLabel}
          </button>
        </div>
      ...
      {cardState === 'panel' && (
        <div
          ...
          style={panelStyle}
          className={`fixed z-50 max-h-[380px] max-w-[520px] overflow-y-auto rounded-2xl border-2 border-accent/40 bg-bg-surface p-5 shadow-[0_8px_24px_rgba(26,29,46,0.14)] ${motion}`}
        >
      ...
      {cardState === 'closed' && (
        <button
          ...
          className={`focus-ring fixed z-50 top-4 left-5 flex items-center gap-1.5 rounded-full border border-[rgba(45,95,76,0.3)] bg-accent-soft px-4 py-2 font-sans text-xs font-medium tracking-widest text-[#1f4d3a] hover:bg-accent hover:text-bg-primary md:top-6 md:left-8 ${motion}`}
        >
